...
static PROCESS_HANDLER_PROTO(processUiHandler)
{
    ...
    if(eventCurrent.event == p46Pressed){
        ledGuiHandler(process, eLedP43, TRUE);
    }else{
        ledGuiHandler(process, eLedP43, FALSE);
    }
    
    if(eventCurrent.event == p70Pressed){
        ledGuiHandler(process, eLedP44, TRUE);
    }else{
        ledGuiHandler(process, eLedP44, FALSE);
    }
    
    if(eventCurrent.event == p71Pressed){
        ledGuiHandler(process, eLedP45, TRUE);
    }else{
        ledGuiHandler(process, eLedP45, FALSE);
    }
}
